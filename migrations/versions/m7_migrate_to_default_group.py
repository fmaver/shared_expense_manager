...
def upgrade() -> None:
    ...
    conn.execute(text("INSERT INTO groups (name, status, group_type) VALUES ('Fran & Guada', 'active', 'regular')"))
    ...
    conn.execute(
        text(f"INSERT INTO group_memberships (group_id, member_id) SELECT {default_group_id}, id FROM members")
    )
    ...
    op.create_unique_constraint("uq_monthly_shares_group_year_month", "monthly_shares", ["group_id", "year", "month"])
