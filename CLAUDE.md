...
- `POST /{group_id}/archive` / `POST /{group_id}/unarchive` — archive **for the calling member only**; archiving is blocked while they carry a balance
- `GET /?archived=true` — that member's archived groups
